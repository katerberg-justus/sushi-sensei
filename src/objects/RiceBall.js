...
export class RiceBall extends IngredientObject {
  constructor(scene, x, y) {
    ...
    this.sprite = scene.add.image(0, 0, RICE_BALL_KEY);
  ...
  static createTexture(scene) {
    if (scene.textures.exists(RICE_BALL_KEY)) {
    ...
    const width = 30;
    const height = 26;
    const texture = scene.textures.createCanvas(RICE_BALL_KEY, width, height);
    const context = texture.getContext();

    context.imageSmoothingEnabled = false;
    context.clearRect(0, 0, width, height);

    context.fillStyle = RiceBall.toHexColor(COLORS.riceStroke);
    ...
    context.fillStyle = RiceBall.toHexColor(COLORS.riceLight);
    context.fillRect(24, 10, 2, 7);
    context.fillRect(22, 17, 3, 3);
    context.fillRect(17, 20, 6, 2);
    context.fillRect(10, 22, 10, 1);

    RiceBall.paintGrain(context, 10, 8, COLORS.riceGrain);
    RiceBall.paintGrain(context, 18, 9, COLORS.riceGrain);
    RiceBall.paintGrain(context, 13, 14, COLORS.riceGrain);
    RiceBall.paintGrain(context, 20, 15, COLORS.riceGrain);
    RiceBall.paintGrain(context, 9, 18, COLORS.riceGrain);
  ...
  static createShadowTexture(scene) {
    ...
    context.fillStyle = RiceBall.toHexColor(0xdddddd);
    ...
    context.fillStyle = RiceBall.toHexColor(0xffffff);
  ...
  static paintGrain(context, x, y, color) {
    context.fillStyle = RiceBall.toHexColor(color);
